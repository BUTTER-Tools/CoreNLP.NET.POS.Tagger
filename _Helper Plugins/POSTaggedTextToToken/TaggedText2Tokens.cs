...
{
    public class POSTaggedTextToTokens : Plugin
    {
        ...
        public string PluginName { get; } = "Helper: POS-Tagged Text to Tokens";
        public string PluginType { get; } = "POS Taggers";
        public string PluginVersion { get; } = "1.0.1";
        public string PluginAuthor { get; } = "Ryan L. Boyd (ryan@ryanboyd.io)";
        public string PluginDescription { get; } = "This plugin take the output of the CoreNLP Part of Speech Tagger and converts it into Tokens. This is a specialized helper that works only as an add-on to the CoreNLP POS Tagger plugin.";
        ...
        public void ChangeSettings()
        {
            ...
            using (var form = new SettingsForm_POSTaggedTextToTokens(TagsOnly))
            {
            ...
            for (int counter = 0; counter < Input.ObjectList.Count; counter++)
                {
                    if (TagsOnly)
                    {
                        pData.StringArrayList.Add(((TaggerOutputObject)Input.ObjectList[counter]).TaggedText_TagsOnly);
                    }
                    else
                    {
                        pData.StringArrayList.Add(((TaggerOutputObject)Input.ObjectList[counter]).TaggedText);
                    }
